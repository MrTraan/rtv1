...
void	init_key_values(t_list2 **key_values, char c_preceed[2])
{
	key_values[0] = NULL;
	key_values[1] = NULL;
	key_values[2] = NULL;
...
t_value	*handle_dict(char *buf, int *i)
{
	...
	while (1)
	{
		...
		(*i)++;
		ft_lstaddend2(&(kv[1]), new_list_elt(tmp_k, (len + 1) * sizeof(char)));
		kv[0] = kv[0] ? kv[0] : kv[1];
		buf[*i] != ':' ? no_colon() : (void)len;
		tmp_value = handle_buf(buf, i);
		ft_lstaddend2(&(kv[3]), new_list_elt(tmp_value, sizeof(t_value)));
		kv[2] = kv[2] ? kv[2] : kv[3];
		if (buf[*i] != ',')
			break ;
	}
...
t_value	*values_list_to_array(t_list2 *values, int nb)
{
	t_value	*res;
	t_value	**data;
	t_list2	*tmp;
...
t_value	*handle_array(char *buf, int *i)
{
	...
	{
		tmp_value = handle_buf(buf, i);
		ft_lstaddend2(&values_end, new_list_elt(tmp_value, sizeof(t_value)));
		if (!values)
			values = values_end;
		if (buf[*i] != ',')
			ret = 0;
